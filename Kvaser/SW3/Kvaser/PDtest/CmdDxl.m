...
for cnt = 1:length( Payload) 
    stat = KvaserCom( 7 , [1536+TargetCanId ,1408+TargetCanId ,nobj,19+cnt,DataType.long,0,100], fix(Payload(cnt)) ); % Set bytes offset 
end
if stat , if nargout < 1 , error ('Sdo failure') ; end  ; return ; end 
...
stat = KvaserCom( 7 , [1536+TargetCanId ,1408+TargetCanId ,nobj,100,DataType.long,0,100], IdOut ); % Set bytes offset 
if stat , if nargout < 1 , error ('Sdo failure') ; end  ; return  ; end
