...
% Last Modified by GUIDE v2.5 21-Mar-2022 20:19:49
...
handles.CalibInit = SaveCalibMan ( [], []  ) ; 
...
global DataType
    ...
    if ~isempty(strFuck) 
        ...
        if isequal(dcd , 'Yes')                    
            ...
            SendObj( [hex2dec('2303'),1] ,hex2dec('12345678') , DataType.long , 'Password for calib') ;
            SendObj( [hex2dec('2303'),2] ,calibRslt.RDoorCenter , DataType.float , 'RDoorCenter calib') ;
            SendObj( [hex2dec('2303'),3] ,calibRslt.RDoorGainFac , DataType.float , 'RDoorGainFac calib') ;
            SendObj( [hex2dec('2303'),4] ,calibRslt.LDoorCenter , DataType.float , 'LDoorCenter calib') ;
            SendObj( [hex2dec('2303'),5] ,calibRslt.LDoorGainFac , DataType.float , 'LDoorGainFac calib') ;
            % apply calibration 
            SendObj( [hex2dec('2303'),29] ,0 , DataType.float , 'Apply calibration') ;
            good = 1 ;
        else
...
SaveCalibMan( handles.CalibInit,[PathName,fname] ) ;
dcd = questdlg('Save to flash?', ...
                         'Please specify', ...
                         'Yes', 'No' , 'No');  
                     
if isequal( dcd , 'Yes' )                 
    ProgCalibMan(fname, 1   ); 
end 
...
% global DataType
% global TargetCanId
% stat = KvaserCom( 7 , [1536+TargetCanId ,1408+TargetCanId ,hex2dec('2303'), 1,DataType.long,0,100], hex2dec('12345678') );  
% if stat , error ('Sdo failure') ; end 
% stat = KvaserCom( 7 , [1536+TargetCanId ,1408+TargetCanId ,hex2dec('2303') ,31,DataType.long,0,100],handles.CalibInit.CalibData );  
% if stat , error ('Sdo failure') ; end 
