function  Calib = ProgCalibVolt(fname, burn  )
% function  Calib = ProgCalibVolt(fname, burn  )
...
SendObj( [CalibObj,1] , hex2dec('12345678') , DataType.long , 'Flash op password' ) ;
for cnt = 1 : length(fieldNames) 
    SendObj( [CalibObj,1+cnt] , Calib.(fieldNames{cnt}) , DataType.float , ['Set calib par [',fieldNames{cnt},']'] ) ;
end

stat = KvaserCom( 7 , [1536+TargetCanId ,1408+TargetCanId ,CalibObj ,20,DataType.long,0,100], Calib.CalibDate  );  
if stat , error ('Sdo CalibDate failure') ; end 

stat = KvaserCom( 7 , [1536+TargetCanId ,1408+TargetCanId ,CalibObj ,21,DataType.long,0,100], Calib.CalibData  );  
if stat , error ('Sdo CalibData failure') ; end 

SendObj( [CalibObj,33] ,Calib.CalibData  , DataType.long , 'Sdo Burn calib failure' ,[] , 300) ;
