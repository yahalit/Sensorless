...
angdeg  = 10 ; % Angular rotation 
amaxdeg = 30  ; % Angular acceleration 
% End user parameters 

deg2rad = pi / 180 ; 
dang = angdeg * deg2rad ; 
vmax = GetFloatPar('AutomaticRunPars.DiffModeSpeed') ; % vmaxdeg * deg2rad ;
amax = GetFloatPar('AutomaticRunPars.DiffModeAcc') ; % amaxdeg  * deg2rad; % Max 3 
...
%SendObj( [hex2dec('2220'),35] , 1 , DataType.long , 'Disable further drift compensation ' ) ;
...
RecNames = {'AzInt','RawYaw','RightWheelEncoder','LeftWheelEncoder','EulerYaw'} ; 
...
plot( r.t , unwrap(r.AzInt - r.AzInt(1)) , r.t , unwrap(r.RawYaw-r.RawYaw(1)),r.t,r.EulerYaw-r.EulerYaw(1),'.'); 
legend('Az int','RawYaw','EulerYaw')
grid on
