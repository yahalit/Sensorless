...
short udpRead(int handle, unsigned long *cobId, char *data, unsigned long * dataLen)
{
	...
	stat = mexCallMATLAB(2, pOut, 0 , NULL , "ReadNewUdp");

	//pOut[0] = mxCreateDoubleMatrix(1, 1, mxREAL); mxGetPr(pOut[0])[0] = 0;
	//pOut[1] = mxCreateDoubleMatrix(1, 1, mxREAL); mxGetPr(pOut[1])[0] = 0;
	...
	if (stat == 0 && (mxGetM(pOut[0])*mxGetN(pOut[0])) )
	{
		...
		{
			MesLen = mxGetM(pOut[1])*mxGetN(pOut[1]); 
			if (MesLen  == 14) 
			{
				//StrStat = mxGetString(pOut[1], RMsg.PayLoad, MesLen);
				SpiErr = (long)mxGetPr(pOut[1])[0] + ((long)mxGetPr(pOut[1])[1] << 8);
				if (SpiErr)
				{
					return -1; 
				}
				CanStat = (long)mxGetPr(pOut[1])[2] + ((long)mxGetPr(pOut[1])[3] << 8);
				if (CanStat)
				{
					//return -1;
				}

				*cobId = ((long)mxGetPr(pOut[1])[4] + ((long)mxGetPr(pOut[1])[5] << 8)) & 0x7ff ;

				for (cnt = 0; cnt < 8; cnt++)
				{
					data[cnt] = mxGetPr(pOut[1])[cnt+6];
					*dataLen = 8; 
				}
				RetVal = 1; 
			}
...
short udpWrite1(int handle, short RxSdoId, char * data)
{
	return 0; 
}
...
short udpWrite(int handle, short RxSdoId,  char * data)
{
	...
	pSrc = (char *)& WMsg; 
	for (cnt = 0; cnt < dimsa[1]; cnt++)
	{
		pChar[cnt ] = pSrc[cnt];
	}
	stat = mexCallMATLAB(0, NULL, 1, &pIn, "WriteNewUdp");
...
int canFlushReceiveQueue(int Handle)
{
	...
	stat = mexCallMATLAB(0, NULL, 0, NULL, "FlushUdpInQueue");
...
*/
		...
		// Write SDO 
		stat = 0; //  
		udpWrite(KvaserStr.Handle, pSdo->RxSdoId, (char *)pSdo->sdata);
		pSdo->State = 1; 
		pSdo->RetryCnt = 0; 
		pSdo->TransferType = 0 ; 
		...
		n1 = 3; //  (short unsigned)(pSdo->TimeOut);
		n = n1 ; 
		while ( n > 0 ) 
		{
			n -= 1 ; 
