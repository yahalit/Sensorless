function sList = ReadSigList( fname , IsSimulation , slavedrive )
...
while (1) 
    ...
    if ( IsSimulation == 0 ) 
        if  contains( s , '#ifdef') && (contains( s , '_LPSIM') || contains( s , 'SIMULATION_MODE'))  
            break; 
        end 

       if  contains( s , '#ifdef') &&  contains( s , 'SLAVE_DRIVER')  
           inslavedrive = 1 ; 
       end
       if inslavedrive && contains( s , '#else')
           inslavedrive  = 0 ; 
           inmasterdrive = 1  ;
       end 
       if (inslavedrive || inmasterdrive) &&  contains( s , '#endif')
           inslavedrive = 0 ; 
           inmasterdrive = 0  ;           
       end 
       if inslavedrive && (slavedrive==0) 
           continue ; 
       end
       if inmasterdrive &&  slavedrive 
           continue ; 
       end
...
%     end
    ...
    NextCell = { flags , strtrim(s(place+1:end)) , groupstr , helpstr }; 
    if isempty ( sList )
        sList = {NextCell }; 
    else
