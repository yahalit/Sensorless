...
[filename, pathname] = ...
     uigetfile({'*.hex'},'Selct FW file',fname);
...
switch ProjType 
    case hex2dec('8000') 
        Operational = 0 ; 
        disp('Found LP boot software' ) ; 
        disp(['Boot FW ver: ',ShowVersionString(FwVer) ]  ) ; 
    case hex2dec('8001') 
        Operational = 0 ; 
        disp('Found PD boot software' ) ; 
        disp(['Boot FW ver: ',ShowVersionString(FwVer) ]  ) ; 
    case hex2dec('8100') 
...
for blcnt = nblocks:-1:1  
    ...
    disp( ['Programming internal block, start at 0x',dec2hex(StartAdd), '  Wait: Block takes about 12 sec' ]) ; 
