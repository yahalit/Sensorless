...
switch Axis 
    case 'Neck' 
        ExpMng = struct('Kp',0.87,'Ki',1.e-3,'CurLevel',1,'F1',100,'F2',1500,'nPoints',20,'maxDiv',3) ; 
...
SetCfgPar('CommutationMode',RecStruct.Enums.CommutationModes.COM_ENCODER_RESET) ;
...
%Ts = FetchObj 0x2221.35
Ts = FetchObj([hex2dec('2221'),35],DataType.float,'Sampling time') ;
...
TestStructFields(ExpMng,{'Kp','Ki','CurLevel','F1','F2','nPoints','maxDiv'},'ExpMng') ; 

% Get Vdc 
Vdc = GetSignal('Vdc') ; 

% Pick frequncies
[fout,Ncyc] = FPicker( ExpMng.F1,ExpMng.F2, ExpMng.nPoints, ExpMng.maxDiv , Ts ) ; 
...
SendObj([hex2dec('2220'),8],RecStruct.Enums.LoopClosureModes.E_LC_Speed_Mode,DataType.long,'Set the loop closure mode to open loop commutation') ;                      
